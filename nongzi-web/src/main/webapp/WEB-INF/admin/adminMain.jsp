...
<head>
    ...
    <!-- 引入EasyUI -->
    <script type="text/javascript"
            src="${pageContext.request.contextPath}/js/views/adminJs/libs/jquery.easyui.min.js"></script>
...
<div data-options="region:'north'" style="height:80px;text-align: center;">
    <br>
    <span style="font-size:30px;color: #1d1d1d;">农资后台管理系统</span>
            ...
            <div title="${list.menuName }" data-options="selected:true">
                <div style="margin: 5px">
                    <ul class="tree easyui-tree" data-options="animate:true,lines:true">
                        <c:forEach items="${adminChildMenu}" var="listChild">
                            <c:if test="${listChild.parentid==list.id}">
                                <li data-options="iconCls:'icon-group_add'">
                                    <a style="text-decoration:none;"
                                       href="${pageContext.request.contextPath}${listChild.childMenuUrl}"
                                       target="center"><span>${listChild.childMenuName}</span></a>
                                </li>
                            </c:if>
                        </c:forEach>
                    </ul>
...
<div data-options="region:'center',title:'内容'">
    <iframe frameborder="0" height="100%" width="100%" scrolling="no" name="center"></iframe>
</div>
</body>
